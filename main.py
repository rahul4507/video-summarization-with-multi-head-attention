...
class AONet:
    ...
    def train(self, output_dir='EX-0'):
        ...
        log_dir = os.path.join(output_dir, 'logs1', f'split_{self.split_id}')
        os.makedirs(log_dir, exist_ok=True)
        
        # Initialize single comprehensive log file
        ...
        for epoch in range(self.hps.epochs_max):
            ...
            # Log epoch header
            log_file.write(f"\n{'='*80}\n")
            log_file.write(f"EPOCH {epoch}/{self.hps.epochs_max}\n")
            log_file.write(f"{'='*80}\n\n")
            ...
            for i, key in enumerate(train_keys):
                ...
                arr = seq.copy()  # Create a copy to avoid modifying original data
                ...
                # Forward pass with motion features
                scores, attention_weights = self.model(seq, seq_len, mn)
                ...
                log_file.write(f"Input seq: {seq.shape}\n")
                log_file.write(f"Motion features: {mn.shape}\n")
                log_file.write(f"Scores: {scores.shape}\n")
                log_file.write(f"Attention weights: {attention_weights.shape}\n")
                
                # Log scores
                scores_np = scores.detach().cpu().numpy()
                log_file.write("\nScore Statistics:\n")
                log_file.write(f"Min: {np.min(scores_np):.6f}\n")
                log_file.write(f"Max: {np.max(scores_np):.6f}\n")
                log_file.write(f"Mean: {np.mean(scores_np):.6f}\n")
                log_file.write(f"Std: {np.std(scores_np):.6f}\n")
                
                # Log score distribution
                hist, bins = np.histogram(scores_np, bins=10)
                log_file.write("\nScore Distribution:\n")
                for i in range(len(hist)):
                    log_file.write(f"Bin {bins[i]:.3f}-{bins[i+1]:.3f}: {hist[i]}\n")
                
                # Log attention statistics
                att_weights_np = attention_weights.detach().cpu().numpy()
                log_file.write("\nAttention Statistics:\n")
                log_file.write(f"Min: {np.min(att_weights_np):.6f}\n")
                log_file.write(f"Max: {np.max(att_weights_np):.6f}\n")
                log_file.write(f"Mean: {np.mean(att_weights_np):.6f}\n")
                log_file.write(f"Std: {np.std(att_weights_np):.6f}\n")
                
                # Log correlation
                try:
                    scores_flat = scores_np.flatten()
                    att_flat = att_weights_np.flatten()
                    min_len = min(len(scores_flat), len(att_flat))
                    scores_flat = scores_flat[:min_len]
                    att_flat = att_flat[:min_len]
                    correlation = np.corrcoef(scores_flat, att_flat)[0,1]
                    log_file.write(f"\nScore-Attention Correlation: {correlation:.6f}\n")
                except Exception as e:
                    log_file.write(f"\nWarning: Could not compute correlation: {str(e)}\n")
                
                # Log top 5 frames
                scores_flat = scores_np.flatten()
                top_5_indices = np.argsort(scores_flat)[-5:][::-1]
                log_file.write("\nTop 5 Frames:\n")
                for idx in top_5_indices:
                    att_value = att_weights_np.flatten()[idx] if idx < len(att_weights_np.flatten()) else "N/A"
                    log_file.write(f"Frame {idx}: Score = {scores_flat[idx]:.6f}, "
                                 f"Attention = {att_value}\n")
                
                # Compute loss
                loss, loss_info = self.model.compute_loss(scores, target, attention_weights)
                
                self.optimizer.zero_grad()
                loss.backward()
                
                if self.hps.clip_grad_norm > 0:
                    torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.hps.clip_grad_norm)
                
                self.optimizer.step()
                
                # Log losses
                log_file.write("\nLosses:\n")
                log_file.write(f"Total Loss: {loss.item():.6f}\n")
                log_file.write(f"Temporal Loss: {loss_info['temp_loss']:.6f}\n")
                log_file.write(f"Diversity Loss: {loss_info['diversity_loss']:.6f}\n")
                
                avg_loss.append([loss.item(), loss_info['temp_loss'], loss_info['diversity_loss']])
                log_file.write("\n" + "=" * 40 + "\n")
            ...
            epoch_loss = np.mean(avg_loss[:, 0])
            
            # Log epoch summary
            log_file.write(f"\nEpoch Summary:\n")
            log_file.write(f"Average Total Loss: {epoch_loss:.6f}\n")
            log_file.write(f"Average Temporal Loss: {np.mean(avg_loss[:, 1]):.6f}\n")
            log_file.write(f"Average Diversity Loss: {np.mean(avg_loss[:, 2]):.6f}\n")
            log_file.write(f"Validation F-score: {val_fscore:.6f}\n")
            log_file.write(f"Learning Rate: {self.optimizer.param_groups[0]['lr']:.6f}\n")
            ...
            print("   Train loss: {0:.05f}".format(epoch_loss), end='')
        ...
        # Plot F-score curve
        plt.subplot(1, 2, 2)
        plt.plot(epochs, val_fscores, 'r-', label='Validation F-score')
        plt.title('Validation F-score over Time')
        plt.xlabel('Epoch')
        plt.ylabel('F-score')
        plt.legend()
